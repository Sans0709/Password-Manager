...
with sqlite3.connect("table.db") as db:  # Connect database
    cursor = db.cursor()
...
def firstTimeScreen():
    ...
    def checkIfUsernameExists():
        ...
        try:
            ...
            label3.config(
                text="Account with this username already exists.\n Enter a different username or login instead.")
    ...
    btn1 = Button(win, text="Sign in", command=loginScreen, bg=bgColor, fg=fgColor, borderwidth=0,
...
def loginScreen():
    ...
    def getMasterPassword():
        ...
        checkHashedPassword = encrypt(
            HashedPassword)  # to convert into encrypted form as passwords are stored in encrypted form in database
        cursor.execute("SELECT * FROM masterpassword WHERE password = ?",
                       [(checkHashedPassword)])  # to select data where the entered password is present
...
def passwordVault():
    ...
    def removeEntry(input):
        cursor.execute("DELETE FROM {0} WHERE ID = ?".format(LoginId),
                       (input,))  # deletes the data of whole row , i.e. particular
    ...
    if (cursor.fetchall() != None):  # if no data is present in table user
        ...
        while (True):
            ...
            btn.grid(column=3, row=i + 3, pady=10, ipadx=13,
                     ipady=3)  # partial calles the function and passes argument in it
